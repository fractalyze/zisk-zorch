...
def _rand_cubic(shape, seed: int) -> fnp.ndarray:
    n = int(np.prod(shape))
    flat = np.random.default_rng(seed).integers(0, 1 << 30, (n, 3)).astype(np.uint64)
    return fnp.array(flat.astype(F).view(F3).reshape(n)).reshape(shape)
...
def _coset_evals(coeffs: fnp.ndarray) -> fnp.ndarray:
    ...
    cols = [fnp.sum(coeffs[m][None, :] * xpow, axis=1) for m in range(coeffs.shape[0])]
    return fnp.stack(cols, axis=1)  # (N_ext, M) cubic


def _poly_eval(coeffs_row: fnp.ndarray, point: fnp.ndarray) -> fnp.ndarray:
    return fnp.sum(coeffs_row * powers(point, coeffs_row.shape[0]))


def _high_coeffs(f: fnp.ndarray) -> np.ndarray:
...
def _limbs(cubic_scalar: fnp.ndarray) -> fnp.ndarray:
