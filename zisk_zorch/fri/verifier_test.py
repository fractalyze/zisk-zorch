...
post-fold transcript via `sample_query_positions`, and the verifier independently
re-derives them — a valid proof verifies only if the two derivations agree.
"""
...
_NONCE = 0x1234  # deferred PoW witness (grinding search out of scope, see queries.py)
...
class FriVerifierTest(absltest.TestCase):
    def _prove(self, case: dict):
        ...
        # real STARK prover would, then open them.
        indices = sample_query_positions(
            transcript,
            proof.final_pol,
            _NONCE,
        ...
        return proof, prove_queries(proof, indices)

    def _verify(self, case, roots, final_pol, openings) -> bool:
        transcript = Transcript()
        transcript.put(u64(case["seed"]))
        return verify(
            roots,
    ...
    def test_accepts_valid_and_rejects_tampering(self) -> None:
        for case in load(_TESTDATA / "fri_prove.json")["cases"]:
            with self.subTest(steps=case["steps"], arity=case["arity"]):
                proof, openings = self._prove(case)

                self.assertTrue(
                    self._verify(case, proof.roots, proof.final_pol, openings),
                ...
                self.assertFalse(
                    self._verify(case, proof.roots, bad_final, openings),
                ...
                self.assertFalse(
                    self._verify(case, proof.roots, proof.final_pol, bad_openings),
