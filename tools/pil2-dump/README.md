...
Reproducibly (re)generates per-stage reference dumps of a real proof, by
running **one real pil2-proofman `genProof` in-process** with the pinned
fork's dump hooks armed. A dumped instance is what a byte-match gate or an
end-to-end benchmark needs to check or time our stages against; nothing in
this repo consumes one yet. A single Rust crate
...
Nothing is committed: build a bundle once per machine by copying an
instance's `*.npy` plus the AIR's `.starkinfo.json`,
`.expressionsinfo.json`, and `.const` into one directory. The prove is
deterministic, so two
